defmodule Membrane.H264.Parser.SchemeParser.State do
  @moduledoc """
  This module defines a structure which holds the state of the scheme parser.
  """
  use Bunch.Access

  @typedoc """
  A type defining the state of the scheme parser. The parser preserves its state in the map, which consists of two parts:
  ...
  * a map under the `:__local__` key -  it holds information valid during a time of a single NALu processing, and it's cleaned after the NALu is
  completly parsed.
  All information fetched from binary part is put into the `:__local__` map.
