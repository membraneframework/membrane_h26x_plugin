defmodule Membrane.H264.Parser.Schemes.SPS do
  def scheme,
    do: [
      ...
      if:
        {{fn profile_idc -> profile_idc in [100, 110, 122, 244, 44, 83, 86, 118, 128] end,
          [:profile_idc]},
         field: {:chroma_format_idc, :ue},
         if:
           {{fn chroma_format_idc -> chroma_format_idc == 3 end, [:chroma_format_idc]},
         ...
         if:
           {{fn seq_scaling_matrix_present_flag -> seq_scaling_matrix_present_flag == 1 end,
            ...
            for:
              {{:i, fn chroma_format_idc -> if chroma_format_idc != 3, do: 8, else: 12 end,
               ...
               if:
                 {{fn seq_scaling_list_present_flag -> seq_scaling_list_present_flag == 1 end,
      ...
      if:
        {{fn pic_order_cnt_type -> pic_order_cnt_type == 0 end, [:pic_order_cnt_type]},
         field: {:log2_max_pic_order_cnt_lsb_minus4, :ue}},
      if:
        {{fn pic_order_cnt_type -> pic_order_cnt_type == 1 end, [:pic_order_cnt_type]},
         ...
         for:
           {{:i,
             fn num_ref_frames_in_pic_order_cnt_cycle ->
               num_ref_frames_in_pic_order_cnt_cycle
             end, [:num_ref_frames_in_pic_order_cnt_cycle]},
      ...
      if:
        {{fn frame_mbs_only_flag -> frame_mbs_only_flag != 1 end, [:frame_mbs_only_flag]},
         field: {:mb_adaptive_frame_field_flag, :u1}},
      field: {:direct_8x8_inference_flag, :u1},
      field: {:frame_cropping_flag, :u1},
      if:
        {{fn frame_cropping_flag -> frame_cropping_flag == 1 end, [:frame_cropping_flag]},
  ...
  def vui_parameters,
    do: [
      ...
      if:
        {{fn aspect_ratio_present_flag -> aspect_ratio_present_flag == 1 end,
          [:aspect_ratio_present_flag]},
         field: {:aspect_ratio_idc, :u8},
         if:
           {{fn aspect_ratio_idc -> aspect_ratio_idc == 255 end, [:aspect_ratio_idc]},
            field: {:sar_width, :u16}, field: {:sar_height, :u16}}},
      field: {:overscan_info_present_flag, :u1},
      if:
        {{fn overscan_info_present_flag -> overscan_info_present_flag == 1 end,
          [:overscan_info_present_flag]}, field: {:overscan_appropriate_flag, :u1}},
      field: {:video_signal_type_present_flag, :u1},
      if:
        {{fn video_signal_type_present_flag -> video_signal_type_present_flag == 1 end,
          [:video_signal_type_present_flag]},
         field: {:video_format, :u3},
         field: {:video_full_range_flag, :u1},
         field: {:colour_description_present_flag, :u1},
         if:
           {{fn colour_description_present_flag -> colour_description_present_flag == 1 end,
      ...
      if:
        {{fn chroma_loc_info_present_flag -> chroma_loc_info_present_flag == 1 end,
      ...
      if:
        {{fn timing_info_present_flag -> timing_info_present_flag == 1 end,
      ...
      if:
        {{fn nal_hrd_parameters_present_flag -> nal_hrd_parameters_present_flag == 1 end,
          [:nal_hrd_parameters_present_flag]}, hrd_parameters()},
      field: {:vcl_hrd_parameters_present_flag, :u1},
      if:
        {{fn vcl_hrd_parameters_present_flag -> vcl_hrd_parameters_present_flag == 1 end,
          [:vcl_hrd_parameters_present_flag]}, hrd_parameters()},
      if:
        {{fn nal_hrd_parameters_present_flag, vcl_hrd_parameters_present_flag ->
            nal_hrd_parameters_present_flag == 1 or vcl_hrd_parameters_present_flag == 1
          end, [:nal_hrd_parameters_present_flag, :vcl_hrd_parameters_present_flag]},
         field: {:low_delay_hrd_flag, :u1}},
      field: {:pic_struct_present_flag, :u1},
      field: {:bitstream_restriction_flag, :u1},
      if:
        {{fn bitstream_restriction_flag -> bitstream_restriction_flag == 1 end,
  ...
  def hrd_parameters(),
    do: [
      ...
      for:
        {{:schedSeiIdx, fn cpb_cnt_minus1 -> cpb_cnt_minus1 + 1 end, [:cpb_cnt_minus1]},
