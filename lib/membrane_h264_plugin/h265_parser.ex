  ...
  * hev1, `:hev1` - The same as hvc1, only that parameter sets may be also present in the stream (in-band).
  ...
  def_options vpss: [
              ...
              generate_best_effort_timestamps: [
                ...
                small, constant latency. The buffer depth is taken from the SPS
                `sps_max_num_reorder_pics`, so when the stream can't reorder frames
                (that value being 0) the buffering is disabled and no latency is added.
